...
R=$ROOT/nvs
BASE="select.methods.mot13b_full=false preds.fair=$R/preds_ss10 results.fair=$R/results_ss10"
cd "$ROOT" || exit 1
END=$(( $(date +%s) + ${1:-180} * 60 ))
while [ "$(date +%s)" -lt "$END" ]; do
  N=$(ls "$R"/results_ss10/*__ss10.csv "$R"/results_ss10/*__ss10b.csv 2>/dev/null | wc -l)
  J=$(squeue -u "$USER" -h -o "%j" | grep -c "__ss10" || true)
  echo "[$(date '+%T')] ss10 scored=$N/160  jobs=$J"
  READY=0
  for SFX in __ss10 __ss10b; do
    OUT=$(timeout 900 "$PY" nvs/slurm/submit_score.py \
          --overrides run.cell_suffix=$SFX $BASE --expect_scenes 10 --allow_partial 2>&1)
    echo "$OUT" | grep -E "^score:|score .*$SFX" | head -8
    R2=$(echo "$OUT" | sed -n 's/^score: \([0-9]*\) cell.*/\1/p')
    READY=$(( READY + ${R2:-0} ))
  done
  if [ "$J" -eq 0 ] && [ "${READY:-1}" -eq 0 ]; then
    echo "[$(date '+%T')] ss10 COMPLETE ($N of 160 cells)"; "$PY" nvs/report/compare_ss10.py; exit 0
  fi
  sleep 240
done
