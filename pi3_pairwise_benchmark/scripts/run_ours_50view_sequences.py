...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run video_world_model 50-view inference on Pi3 Re10K samples.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override VWM model n_frames before construction. Use for exact Pi3 10-view/variable-length smoke tests.",
    )
    ...
    context = {
        ...
        "override_n_frames": args.override_n_frames,
    ...
    for row_idx, row in enumerate(rows):
        try:
            print(
                f"[sequence {row_idx + 1}/{len(rows)}] {row.sequence_name}: "
                f"{len(row.image_paths)} frames inference begin",
                flush=True,
            )
            pred_c2w = np.asarray(
