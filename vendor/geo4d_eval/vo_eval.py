...
def load_sintel_intrinsic(gt_file): # './data/sintel/training/camdata_left/alley_2'
    # Refer to ParticleSfM
    gt_pose_lists = sorted(os.listdir(gt_file))
    gt_pose_lists = [os.path.join(gt_file, x) for x in gt_pose_lists if x.endswith(".cam")]
    tstamps = [float(x.split("/")[-1][:-4].split("_")[-1]) for x in gt_pose_lists]
    gt_intrinsics = [sintel_cam_read(f)[0] for f in gt_pose_lists] # [0] means get the intrinsic
    return gt_intrinsics
    

def load_intrinsic(gt_file, traj_format="sintel"):
    assert traj_format == "sintel"
    gt_intrinsics = load_sintel_intrinsic(gt_file)
    gt_intrinsics = np.array(gt_intrinsics)
    return gt_intrinsics
...
def align_trajectory_with_eval(pred_traj, gt_traj, correct_scale=True, return_aligned_traj=False, align_origin=False):
    pred_traj = make_traj(pred_traj)
    ...
    if align_origin:
        assert correct_scale == False
        P = pred_traj.align_origin(gt_traj)
        ...
        rpe_trans, rpe_rot = np.mean(rpe_transs), np.mean(rpe_rots)
        if return_aligned_traj:
            return ate, rpe_trans, rpe_rot, P, pred_traj
        else:
            return ate, rpe_trans, rpe_rot, P


    else:
        r_a, t_a, s = geometry.umeyama_alignment(pred_traj.positions_xyz.T,
        ...
        if return_aligned_traj:
            ...
            return ate, rpe_trans, rpe_rot, r_a, t_a, s, pred_traj

    return ate, rpe_trans, rpe_rot, r_a, t_a, s
